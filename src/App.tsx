import {Box, ChakraProvider, Divider, Input, InputGroup, InputLeftElement, Spinner, Stack,} from '@chakra-ui/react';
...
import {setObservableState} from './state/state-utils';
import {
  fileStore,
  filteredRowsByType,
  setFileFromObjectFn,
...
function App() {
  ...
  useEffect(() => {
    const subscription = fromEvent(window, 'resize').pipe(
      ...
      tap(() => setObservableState(UIState$, () => ({innerWidth: window.innerWidth})))
    ).subscribe();
    return () => subscription.unsubscribe();
  }, []);

  const virtualTableHeight = useCallback(() => {
    return (appContainerRef.current?.clientHeight || 0) - (headerContainerRef.current?.clientHeight || 0) - 15;
  }, []);
      ...
      <ChakraProvider>
        <AppPage>
          ...
          >
            ...
            {(fileState.loading || (fileState.rows.length > 0 || fileState.columns.length > 0)) && (
              <Box mt={2} mb={2}>
                {fileState.loading
                  ? (
                    <Box
